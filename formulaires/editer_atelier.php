...
function formulaires_editer_atelier_charger_dist($id_atelier='new', $id_rubrique=0, $retour='', $lier_trad=0, $config_fonc='', $row=array(), $hidden=''){
	...
	if (!_request('enregistrer')) {	
		...
		$autres_organisations_france = sql_allfetsel('id_organisation', 'spip_organisations_liens', 
																'id_objet='.intval($id_atelier).' AND objet="atelier"');
	...
	$valeurs['cle_ajouter_document'] = calculer_cle_action('ajouter-document-atelier-' . $id_atelier);
 ...
 */
function formulaires_editer_atelier_identifier_dist($id_atelier='new', $id_rubrique=0, $retour='', $lier_trad=0, $config_fonc='', $row=array(), $hidden=''){
	return serialize(array(intval($id_atelier)));
}
...
function formulaires_editer_atelier_verifier_dist($id_atelier='new', $id_rubrique=0, $retour='', $lier_trad=0, $config_fonc='', $row=array(), $hidden=''){
	
    $erreurs = formulaires_editer_objet_verifier('atelier', $id_atelier, array('id_ue_edition', 'nom_referent', 'prenom_referent', 'organisation_referent', 'adresse_referent', 'code_postal_referent', 'ville_referent', 'email_referent', 'email_referent_confirmation', 'telephone_referent', 'id_organisation', 'titre_court', 'presentation'));
    ...
    if (isset($_FILES['ajouter_document'])
        ...
        if (_request('cle_ajouter_document')!=calculer_cle_action($a = "ajouter-document-atelier-$id_atelier")){
...
function formulaires_editer_atelier_traiter_dist($id_atelier='new', $id_rubrique=0, $retour='', $lier_trad=0, $config_fonc='', $row=array(), $hidden=''){

	$res = formulaires_editer_objet_traiter('atelier',$id_atelier,$id_rubrique,$lier_trad,$retour,$config_fonc,$row,$hidden);

	sql_updateq('spip_ateliers', 
		array('id_organisation' => _request('id_organisation'), 'id_ue_edition' => _request('id_ue_edition')), 
		'id_atelier='.$res['id_atelier']);
	
	sql_delete('spip_organisations_liens', 'id_objet='.$res['id_atelier'].' AND objet="atelier"');
	
	foreach(_request('autres_organisations_france') as $id_organisation) {
		sql_insertq('spip_organisations_liens', array(
													'id_organisation' => $id_organisation,
													'id_objet' => $res['id_atelier'],
													'objet' => 'atelier'));
	}
	
	if (!intval($id_atelier)) {
		sql_updateq('spip_intervenants', array('id_objet' => $res['id_atelier']), 
			'id_objet=-'.intval($GLOBALS['visiteur_session']['id_auteur']).' AND objet="atelier"');
	}
